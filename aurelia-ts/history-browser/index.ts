import {IBrowserHistoryOptions} from './interfaces';
export {IBrowserHistoryOptions} from './interfaces';
...
export class BrowserHistory extends History {
    ...
    public root: string;
    public options: IBrowserHistoryOptions;
    public fragment: string;
    public iframe: Window;
    private _checkUrlCallback: () => boolean;
    ...
    navigate(fragment?: string, options?: boolean | INavigateOptions): boolean {
        ...
        if (this._hasPushState) {
            this.history[(<INavigateOptions>options).replace ? 'replaceState' : 'pushState']({}, document.title, url);

            // If hash changes haven't been explicitly disabled, update the hash
            // fragment to store history.
        } else if (this._wantsHashChange) {
            updateHash(this.location, fragment, (<INavigateOptions>options).replace);

            if (this.iframe && (fragment !== this.getFragment(this.getHash(this.iframe)))) {
                // Opening and closing the iframe tricks IE7 and earlier to push a
                // history entry on hash-tag change.  When replace is true, we don't
                // want history.
                if (!(<INavigateOptions>options).replace) {
                    this.iframe.document.open().close();
                }

                updateHash(this.iframe.location, fragment, (<INavigateOptions>options).replace);
            }

            // If you've told us that you explicitly don't want fallback hashchange-
            // based history, then `navigate` becomes a page refresh.
        } else {
            return <any>this.location.assign(url);
        }

        if ((<INavigateOptions>options).trigger) {
